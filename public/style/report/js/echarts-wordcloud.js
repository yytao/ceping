    ...
    function () {
      ...
      var __webpack_modules__ = {
        ...
        function indexJs(__unused_webpack_module, __webpack_exports__, __webpack_require__) {
          ...
          var getItemExtraData = function getItemExtraData(item) {
            if (Array.isArray(item)) {
              var itemCopy = item.slice(); // remove data we already have (word and weight)
          ...
          var WordCloud = function WordCloud(elements, options) {
            ...
            var putWord = function putWord(item, loopIndex) {
              ...
              var extraDataArray = getItemExtraData(item); // get info needed to put the text onto the canvas

              var info = getTextInfo(word, weight, rotateDeg, extraDataArray); // not getting the info means we shouldn't be drawing this one.
              ...
              var tryToPutWordAtPoint = function tryToPutWordAtPoint(gxy) {
                ...
                drawText(gx, gy, info, word, weight, maxRadius - r, gxy[2], rotateDeg, attributes, extraDataArray); // Mark the spaces on the grid as filled
